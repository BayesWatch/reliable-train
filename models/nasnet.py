# from https://github.com/aussetg/nasnet.pytorch
import torch
...
class CIFARNASNet(nn.Module):
    def __init__(self, normals, filters, scaling, num_classes, use_aux=True, pretrained=True):
        super(CIFARNASNet, self).__init__()
        self.normals = normals
        self.use_aux = use_aux
        self.num_classes = num_classes
...
def imagenet_nasnetmobile(num_classes=1000, pretrained=False):
    return ImageNetNASNet(32, 4, 44, 2, num_classes=num_classes, use_aux=True, pretrained=pretrained)

def imagenet_nasnetlarge(num_classes=1000, pretrained=False):
    return ImageNetNASNet(96, 6, 168, 2, num_classes=num_classes, use_aux=True, pretrained=pretrained)

def cifar_nasnetmobile(num_classes=10, pretrained=False):
    return CIFARNASNet(32, 4, 44, 2, num_classes=num_classes, use_aux=True, pretrained=pretrained)

def cifar_nasnetlarge(num_classes=10, pretrained=False):
    return CIFARNASNet(96, 6, 168, 2, num_classes=num_classes, use_aux=True, pretrained=pretrained)

def test():
    net = imagenet_nasnetmobile()
    y = net(Variable(torch.randn(1,3,224,224)))
    print(y.size())
    net = cifar_nasnetmobile()
